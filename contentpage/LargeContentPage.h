...
class LargeContentPage : public ZQ::common::SharedObject
{
	friend class CPMISERVANT(PageAllocator);
	friend class PageFile;

public:
	typedef ZQ::common::Pointer< LargeContentPage > Ptr;
...
//	typedef std::queue< Ptr > Queue;
	...
	typedef struct _PageDescriptor
	{
		...
		std::string urlAllocator; // the url to the allocator who owns the page
...
protected:
	...
	LargeContentPage(PageManager& sm, PageDescriptor& bufd, const std::string& uriOrigin, int64 offsetOrigin, int HeadLength = 0);

public:

	virtual ~LargeContentPage();
...
{
public:
	static bool junmarshal_PageDescriptor(LargeContentPage::PageDescriptor& pd, const Json::Value& value);
	static void jmarshal_PageDescriptor(const LargeContentPage::PageDescriptor& pd, Json::Value& value);

	DECLARE_CPMI_API(allocate,  (LargeContentPage::PageDescriptor& pd, const std::string& uriOrigin, int64 offsetOrigin, bool prefill_wished), (LargeContentPage::PageDescriptor& pd, const std::string& uriOrigin, int64 offsetOrigin, bool prefill_wished, const std::string& reqtxn));
	DECLARE_CPMI_API(free,      (LargeContentPage::PageDescriptor& pd),                                                                  (LargeContentPage::PageDescriptor& pd, const std::string& reqtxn));
};

DECLARE_STUB_START(PageAllocator)
	DECALRE_CPMICLIENT_API(allocate,  (LargeContentPage::PageDescriptor& pd, const std::string& uriOrigin, int64 offsetOrigin, bool prefill_wished), (LargeContentPage::PageDescriptor& pd, const std::string& uriOrigin, int64 offsetOrigin, bool prefill_wished, const std::string& reqtxn));
	DECALRE_CPMICLIENT_API(free,      (LargeContentPage::PageDescriptor& pd),                                                                  (LargeContentPage::PageDescriptor& pd, const std::string& reqtxn));
...
DECLARE_SERVANT_START(PageAllocator)
	DECALRE_CPMISERVER_API(allocate,  (LargeContentPage::PageDescriptor& pd, const std::string& uriOrigin, int64 offsetOrigin, bool prefill_wished), (LargeContentPage::PageDescriptor& pd, const std::string& uriOrigin, int64 offsetOrigin, bool prefill_wished, const std::string& reqtxn));
	DECALRE_CPMISERVER_API((free,     (LargeContentPage::PageDescriptor& pd),                                                                  (LargeContentPage::PageDescriptor& pd, const std::string& reqtxn));
DECLARE_SERVANT_END(PageAllocator);

// -----------------------------
// class PageRelay as a proxy to forward alloc/free to a remote
// -----------------------------
class PageRelay : public CPMIAPP(PageAllocator)
{
	friend class LargeContentPage;
public:
	PageRelay(ZQ::common::Log& log, ZQ::sloop::InterruptibleLoop& sloop, ZQ::sloop::CPMIService& cpmi);
	virtual ~PageRelay() {}
	...
	virtual LargeContentPage::Ptr allocate(const std::string& uriOrigin, int64 offsetOrigin, bool prefill_wished=false) { return NULL; }

	// free an allocated page
	virtual void free(LargeContentPage::PageDescriptor& bufd)
	{
		CPMICLIENT(APICLS)::Ptr client;
		if (!bufd.urlAllocator.empty()) 
			client= open_remote(bufd.bufd.urlAllocator);
		if (!client) return;
...
protected:
	virtual CPMICLIENT(APICLS)::Ptr open_remote(const std::string& urlAllocator); // may lookup _prxMap first
...
// -----------------------------
// class PageManager that extends PageRelay with local page pool
// -----------------------------
class PageManager : public PageRelay // inherit PageRelay as a forwarder in the case PageDescriptor.urlAllocator refer to remote
{
	friend class LargeContentPage;

public:
	typedef std::list<LargeContentPage::PageDescriptor> SegmentList;
...
public: // overwrite PageRelay
	// LargeContentPage::Ptr allocateEmptyPacket(const std::string& uriOrigin, int64 offsetOrigin);
	// void freeEmptyPacket(LargeContentPage::Ptr seg);
	LargeContentPage::Ptr newNullSegment(const std::string& uriOrigin, int64 offsetOrigin);
	void freeNullSegment(LargeContentPage::Ptr seg);
...
protected:	
	...
	LargeContentPage::Ptr hatchSegObj(LargeContentPage::PageDescriptor& bufd, const std::string& uriOrigin = "default", int64 offsetOrigin = 0)
	{
		if(bufd.key.empty())
			bufd.key = PageManager::genShmkey(uriOrigin, offsetOrigin);

		LargeContentPage::Ptr seg = new SegmentT(*this, bufd, uriOrigin, offsetOrigin);
		return seg;
	}
